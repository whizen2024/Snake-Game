/* Final Advanced Snake - MultiFood, Difficulty, Fullscreen, Responsive
   Updated: high score now stores player's name along with score
*/
...
let speed = 6, score = 0;
/* HIGH SCORE: now an object { score: number, name: string } */
...
let hiscoreObj = (function(){
  try{
    ...
    if(typeof parsed.score === 'number' && typeof parsed.name === 'string') return parsed;
    return { score: 0, name: "" };
...
function spawnFood(val){
  ...
  while(tries++<300){
    ...
    if(!snake.some(s=>s.x===x&&s.y===y)&&!foods.some(f=>f.x===x&&f.y===y)){
      /* give each food a small id so filtering is robust */
      foods.push({x,y,val,id: Math.random().toString(36).slice(2,9)});
      return;
    }
  }
}
...
function gameOver(){
  ...
  // if current score beats stored score, save with the player name
  if(score > hiscoreObj.score){
    const name = (playerNameInput && playerNameInput.value && playerNameInput.value.trim()) ? playerNameInput.value.trim() : "Player";
    const newObj = { score: score, name: name };
    saveHighScoreObject(newObj);
  }

  finalScore.textContent=`Score: ${score}`;
  highScoreEl.textContent = hiscoreObj.score > 0 ? `High: ${hiscoreObj.score} (${hiscoreObj.name || "Player"})` : `High: 0`;
...
overlayStartBtn.onclick=()=>{
  // capture player name early (optional) but final saving happens at game over
  resetGame();
...
restartBtn.onclick=()=>{ resetGame(); running=true; requestAnimationFrame(loop); };
...
/* INIT */
updateScore();  // show any saved highscore & name
resetGame();
render();
